...
const CollectionSelector = ({ onCollectionChange, onSubcategoryChange, user }) => {
    ...
    useEffect(() => {
        const fetchCollections = async () => {
            try {
                if (!token) return;
                const response = await fetch(`http://localhost:5000/collections/parents`, {
    ...
    useEffect(() => {
        if (selectedParent) {
            fetch(`http://localhost:5000/collections/${selectedParent}`, {
                headers: { Authorization: `Bearer ${token}` },
            })
                .then(res => res.json())
    ...
    const handleParentSelection = (name) => {
        ...
        const matched = collections.find(pt => pt.name.toLowerCase() === name.toLowerCase());
        if (matched) {
            setSelectedParent(matched.id);
            onCollectionChange(matched.id);
        } else {
    ...
    const handleAddCollection = async () => {
        ...
        try {
            const response = await fetch("http://localhost:5000/collections", {
                ...
                body: JSON.stringify({ name: newCollection, parent_id: null }),
            });

            if (response.ok) {
                const newType = await response.json();
                setCollections([...collections, newType]);
    ...
    const handleEditSubmit = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setCollections(collections.map(pt => (pt.id === updated.id ? updated : pt)));
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (response.ok) {
                setCollections(collections.filter(pt => pt.id !== deleteId));
        ...
        <div>
            <Dropdown show={showDropdown} ref={dropdownRef}>
                ...
                <Form.Control
                    ...
                    value={newCollection}  // Show selected name
            ...
            {/* Add Button */}
            <Button variant="success" onClick={handleAddCollection}>+</Button>
