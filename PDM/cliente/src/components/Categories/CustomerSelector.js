...
const CustomerSelector = ({ onCustomerChange, user }) => {
    ...
    useEffect(() => {
        const fetchCustomers = async () => {
            try {
                if (!token) return;
                const response = await fetch(`http://localhost:5000/customers/parents`, {
    ...
    useEffect(() => {
        if (selectedParent) {
            fetch(`http://localhost:5000/customers/${selectedParent}`, {
                headers: { Authorization: `Bearer ${token}` },
            })
                .then(res => res.json())
    ...
    const handleParentSelection = (name) => {
        ...
        const matched = customers.find(pt => pt.name.toLowerCase() === name.toLowerCase());
        if (matched) {
            setSelectedParent(matched.id);
            onCustomerChange(matched.id);
        } else {
    ...
    const handleAddCustomer = async () => {
        ...
        try {
            const response = await fetch("http://localhost:5000/customers", {
                ...
                body: JSON.stringify({ name: newCustomer, parent_id: null }),
            });

            if (response.ok) {
                const newType = await response.json();
                setCustomers([...customers, newType]);
    ...
    const handleEditSubmit = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setCustomers(customers.map(pt => (pt.id === updated.id ? updated : pt)));
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (response.ok) {
                setCustomers(customers.filter(pt => pt.id !== deleteId));
        ...
        <div>
            <Dropdown show={showDropdown} ref={dropdownRef}>
                ...
                <Form.Control
                    ...
                    value={newCustomer}  // Show selected name
            ...
            {/* Add Button */}
            <Button variant="success" onClick={handleAddCustomer}>+</Button>
